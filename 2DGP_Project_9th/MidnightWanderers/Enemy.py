...
class Goblin:
    ...
    def update(self, frame_time):
        ...
        if self.comeout == False:
            Goblin.FRAMES_PER_ACTION = enemy_data['Goblin1']['frames_per_action']
            self.clip_width = enemy_data['Goblin1']['clip_width']
            self.clip_height = enemy_data['Goblin1']['clip_height']

            if self.character.x < self.x:
                self.image = load_image('Resources\Enemies\MidnightWanderers_Enemies_LeftGoblin1.png')
                self.dir_x = -1
            else:
                self.image = load_image('Resources\Enemies\MidnightWanderers_Enemies_RightGoblin1.png')
                self.dir_x = 1
            ...
            Goblin.FRAMES_PER_ACTION = enemy_data['Goblin2']['frames_per_action']
            self.clip_width = enemy_data['Goblin2']['clip_width']
            self.clip_height = enemy_data['Goblin2']['clip_height']
